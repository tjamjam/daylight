...
export default function NetworkMonitor() {
  ...
  if (!isOnline) {
    return (
      <div className="mt-8">
        <div className="bg-zinc-950 border border-zinc-700 rounded-xl p-10 text-center">
  ...
  return (
    <div className="mt-8">
      {/* Terminal window */}
      <div className="bg-zinc-950 border border-zinc-700 rounded-xl overflow-hidden font-mono text-sm">
        {/* Title bar */}
        <div className="flex items-center justify-between px-4 py-2.5 bg-zinc-900 border-b border-zinc-700">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-3 text-zinc-400 text-xs">network-firewall — {count} request{count !== 1 ? 's' : ''} blocked</span>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={handleTest}
              disabled={testing}
              className="px-3 py-1 bg-amber-500 text-zinc-900 text-xs font-semibold rounded-md hover:bg-amber-400 transition-colors disabled:opacity-50"
            >
        ...
        <div ref={scrollRef} className="px-4 py-3 min-h-[120px] max-h-[240px] overflow-y-auto">
          <p className="text-green-500 mb-1">
            <span className="text-zinc-500">$</span> Patching window.fetch — intercepting outbound requests
          </p>
          <p className="text-green-500 mb-1">
            <span className="text-zinc-500">$</span> Patching XMLHttpRequest.open — intercepting outbound requests
          </p>
          <p className="text-zinc-400 mb-3">
            <span className="text-zinc-500">$</span> Firewall active. All external requests will be blocked.
          </p>
          ...
          {blockedRequests.length === 0 ? (
            <>
              ...
              <p className="text-zinc-500 mb-1">
                &nbsp;&nbsp;Process images above — nothing will leave your browser.
              </p>
      ...
      <div className="mt-4 flex items-start gap-3">
        ...
        <p className="text-sm text-zinc-300">
          Daylight patches <code className="text-amber-500/80 bg-zinc-800 px-1 py-0.5 rounded text-xs">window.fetch</code> and <code className="text-amber-500/80 bg-zinc-800 px-1 py-0.5 rounded text-xs">XMLHttpRequest</code> to
